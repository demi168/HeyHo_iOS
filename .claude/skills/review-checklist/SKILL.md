...
- [ ] 遅延は `DispatchQueue.main.asyncAfter` ではなく `Task { try? await Task.sleep(...) }`（View 破棄時にキャンセルできるか／長く走るループは `@State` の `Task` に保持して `onDisappear` でキャンセルしているか）

## ログ・エラー処理 → [logging](../logging/SKILL.md)

- [ ] `print()` を使っていないか（`AppLogger.<category>` を使う）
- [ ] `try?` でエラーを握りつぶしていないか（失敗は必ずログか `.errorAlert` に接続）
- [ ] ログの補間値に不要な `.public` を付けていないか（デフォルト private のまま）
...
## Preview・テスト → [testing](../testing/SKILL.md)

- [ ] BodyView は `EnvironmentObject` なしに `#Preview` で表示できるか
- [ ] ローディング・空・データあり の最低 3 ケースを Preview でカバーしているか
- [ ] テストしたい判定/変換ロジックは `Models/`（`Foundation` のみ import）に純粋関数で切り出したか
- [ ] 切り出したロジックは pbxproj で dual membership 登録し、`HeyHoTests/` に Swift Testing でテストを追加したか
- [ ] `xcodebuild test -only-testing:HeyHoTests` が通るか
