...
| スキル | 内容 | こんな時に読む |
|--------|------|--------------|
| [testing](./.claude/skills/testing/SKILL.md) | HeyHoTests のターゲット構成（ホストなし・dual membership）・Swift Testing・テスト可能ロジックの切り出し規約 | テストを追加/実行する時 |
| [logging](./.claude/skills/logging/SKILL.md) | `AppLogger` のカテゴリ・`try?` 握りつぶし禁止・privacy 方針 | ログ追加・エラー処理を書く時 |
